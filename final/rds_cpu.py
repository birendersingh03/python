...
#print(rds_cpu(database,Previous_Date, NextDay_Date ))
...
for database in list_instance:
     cpu_avg = rds_cpu(database,Previous_Date, Current_Date )
     mem_avg = rds_mem(database,Previous_Date, Current_Date ) 
     cursor.execute("UPDATE db1 SET cpu = %s, mem = %s WHERE instance_name = '%s'" % (cpu_avg, mem_avg, database))
     db.commit()
